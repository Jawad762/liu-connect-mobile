...
import type { Comment, CreateCommentMedia } from '@/types/comment.types';
...
const UpdateCommentModal = ({ visible, onRequestClose, comment }: { visible: boolean; onRequestClose: () => void, comment: Comment }) => {
    ...
    const [media, setMedia] = useState<CreateCommentMedia[]>(comment.media.map((m) => ({ url: m.media_url, type: m.type })));
    ...
    useEffect(() => {
        if (visible) {
            ...
            setMedia((comment.media ?? []).map((m) => ({ url: m.media_url, type: m.type })));
        } else {
            setContent('');
            setMedia([]);
        }
            ...
            >
                <View className='flex-row items-center justify-between px-4 pb-3'>
                    ...
                    <GradientButton
                        ...
                        disabled={commentCreationLoading || (comment.content === content && media.length === comment.media.length)}
                        className='min-w-20'
