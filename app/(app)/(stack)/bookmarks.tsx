...
export default function BookmarksScreen() {
  ...
  const { postBookmarks, fetchNextPage, hasNextPage, isFetchingNextPage, refetch, isFetching, error } = usePostBookmarks({ size: 10 });
  ...
  return (
    <ThemedView className='flex-1' style={{ paddingTop: insets.top + 12 }}>
      <View className="p-4">
        ...
        <View className='w-full flex-row justify-center border border-red-500 pt-4'>
      ...
      {selectedTab === "posts" && (
        <PostList posts={postBookmarks} isFetching={isFetching} error={error} refetch={refetch} fetchNextPage={fetchNextPage} hasNextPage={hasNextPage} isFetchingNextPage={isFetchingNextPage} />
      )}
